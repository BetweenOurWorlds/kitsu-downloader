...
function downloadRecursive(currentPage, stopPage, writer) {
  logger.debug(`working on page ${currentPage}`);

  download(20, currentPage * 20).then(res => {
    ...
    writer.write(data).then(() => {
      if (currentPage !== stopPage) {
        downloadRecursive(currentPage + 1, stopPage, writer);
      }
    });
  }).catch(e => {
    console.error(`error during ${currentPage}. Retrying...`);
    downloadRecursive(currentPage, stopPage, writer);
  });
}
               ...
               if (parsedData.included) {
                 ...
                 parsedData.included.forEach(item => {
                   if (item.type === 'characters') {
                     anime.characters.push(item.id);
                   }
                 });
               } else {
                 console.warn(`no characters found for anime with id "${anime.id}"`);
               }
        
               characterDeferred.resolve();
             } catch (e) {
               //console.error(e.message);
               console.error(`error during processing of characters`);
               deferred.reject(e);
             }
           });
         });
...
module.exports = downloadRecursive;
